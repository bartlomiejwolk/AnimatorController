...
public class AnimatorParamDrawer : GameComponentPropertyDrawer {
	...
	public override void OnGUI(
		...
		if (sourceType.enumValueIndex == (int)AnimatorController.SourceTypes.Component) {
			// Draw 'param' field.
			...
			EditorGUI.PropertyField(
					new Rect(
						...
						16),
					param,
					new GUIContent("Param", "Animator parameter name to update."));
			...
			EditorGUI.PropertyField(
					new Rect(
						...
						16),
					trigger,
					new GUIContent("Trigger", "If the animator param. is a trigger."));
		...
		if (sourceType.enumValueIndex == (int)AnimatorController.SourceTypes.Message) {
		...
		if (
				...
				sourceType.enumValueIndex == (int)AnimatorController.SourceTypes.Component) {
			// Get all properties from source component.
			_sourceProperties = sourceCo.objectReferenceValue.GetType().GetProperties();
			// Initialize array.
			...
			sourcePropIndex.intValue = EditorGUI.Popup(
				new Rect(
					pos.x,
					pos.y + (3 * 20), // rows * (row height + empty space)
					pos.width,
					16),
				"Source Prop.",
			...
			sourcePropertyName.stringValue = sourcePropNames[sourcePropIndex.intValue];
		}
	}
}
